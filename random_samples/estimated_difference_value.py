def estimated_difference_value(
    number_of_samples: int, dimensions: int = 2, p_norm: int = 2, repeats: int = 100
) -> np.ndarray:
    ...
    return np.linalg.norm(paired[:, :, 0] - paired[:, :, 1], ord=p_norm, axis=-1).mean(
        axis=0
    )
