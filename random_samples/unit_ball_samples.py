...
def pivot_with_mask(vec: np.ndarray, mask: np.ndarray):
    ...
    size = mask.sum()
    new_samples = np.random.random_sample(size * vec.shape[1]).reshape(
        size, vec.shape[1]
    )
...
def generate_samples(
    quantity: int = 10, dimension: int = 2, p_norm: int | float = 2
) -> np.ndarray:
...
def generate_samples_with_mask_callback(
    quantity: int, dimension: int, mask_callback: Callable[[np.ndarray], Mask]
) -> np.ndarray:
    sample_vec = random_sample(quantity * dimension).reshape((quantity, dimension))
